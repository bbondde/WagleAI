from keras.layers import \
    Conv2D, Input, BatchNormalization, LeakyReLU, ZeroPadding2D, UpSampling2D
...
class YoloLayer(Layer):
    def __init__(self, anchors, max_grid, batch_size, warmup_batches,
                 ignore_thresh, grid_scale, obj_scale, noobj_scale,
                 xywh_scale, class_scale, **kwargs):

        # make the model settings persistent
        self.ignore_thresh = ignore_thresh
        self.warmup_batches = warmup_batches
        self.anchors = tf.constant(anchors, dtype='float',
                                   shape=[1, 1, 1, 3, 2])
        ...
        cell_x = tf.to_float(tf.reshape(tf.tile(
    ...
    def call(self, x):
        ...
        # adjust the shape of the y_predict
        # [batch, grid_h, grid_w, 3, 4+1+nb_class]
        ...
        pred_box_conf = \
            tf.expand_dims(tf.sigmoid(y_pred[..., 4]), 4)  # adjust confidence
        ...
        avg_obj = tf.reduce_sum(
            pred_box_conf * object_mask) / (count + 1e-3)
        ...
        true_box_xy, true_box_wh, xywh_mask = \
            tf.cond(tf.less(batch_seen, self.warmup_batches+1),
                    lambda: [true_box_xy +
        ...
        wh_scale = tf.expand_dims(2 - wh_scale[..., 0] *
                                  wh_scale[..., 1], axis=4)
        # the smaller the box, the bigger the scale

        xy_delta = \
            xywh_mask * (pred_box_xy-true_box_xy) * wh_scale * self.xywh_scale
        wh_delta = \
            xywh_mask * (pred_box_wh-true_box_wh) * wh_scale * self.xywh_scale
        conf_delta = \
            object_mask * (pred_box_conf-true_box_conf) * self.obj_scale + \
...
def create_yolov3_model(
    ...
    pred_yolo_1 = _conv_block(x, [{'filter': 1024, 'kernel': 3, 'stride': 1,
                                  ...
                                  {'filter': (3*(5+nb_class)), 'kernel': 1,
                                   'stride': 1, 'bnorm': False, 'leaky': False,
    ...
    loss_yolo_2 = YoloLayer(anchors[6:12], 
                            ...
                            noobj_scale,
                            xywh_scale,
                            class_scale)([input_image, pred_yolo_2,
    ...
    loss_yolo_3 = YoloLayer(anchors[:6], 
                            ...
                            noobj_scale,
                            xywh_scale,
                            class_scale)([input_image, pred_yolo_3,
                                          true_yolo_3, true_boxes])

    train_model = Model([input_image, true_boxes,
                         true_yolo_1, true_yolo_2, true_yolo_3],
